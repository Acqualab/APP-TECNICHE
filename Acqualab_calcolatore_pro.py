import streamlit as st
import math

# --- CONFIGURAZIONE PAGINA ---
st.set_page_config(page_title="Acqualab PRO - Enterprise Edition", page_icon="🏢", layout="centered")

# --- STILE CSS COMPLETO (Light + PRO) ---
st.markdown("""
    <style>
    .misura-grande { font-size: 36px !important; font-weight: bold; color: #FF4B4B; }
    .valore-evidenziato { font-size: 32px !important; font-weight: bold; color: #00AEEF; }
    .result-box { padding: 20px; border-radius: 12px; border: 1px solid rgba(128, 128, 128, 0.2); background-color: rgba(128, 128, 128, 0.05); margin-bottom: 15px; text-align: center; }
    .titolo-sezione { font-size: 20px; font-weight: bold; color: var(--text-color); border-bottom: 2px solid #00AEEF; margin-bottom: 15px; margin-top: 15px; }
    .stat-label { font-size: 13px; color: gray; font-weight: bold; text-transform: uppercase; }
    .stat-value { font-size: 22px; font-weight: bold; }
    .green-res { color: #28a745; font-weight: bold; }
    </style>
    """, unsafe_allow_html=True)

# --- SIDEBAR ---
try:
    st.sidebar.image("Color con payoff - senza sfondo.png", use_container_width=True)
except:
    st.sidebar.title("ACQUALAB S.R.L.")

st.title("🧪 Suite Calcoli PRO")

# Definizione sicura dei Tab per evitare NameError
tab1, tab2, tab3, tab4 = st.tabs(["🏊 Piscina", "💧 Soluzione", "🚰 Gestione", "📐 Progetto UNI 9182"])

# --- TAB 1: PISCINA (Dosaggi Completi) ---
with tab1:
    st.header("Analisi e Interventi Piscina")
    c1, c2 = st.columns(2)
    with c1:
        v_piscina = st.number_input("Volume Piscina (m³)", min_value=1.0, value=100.0)
        ph_ril = st.number_input("pH Rilevato", min_value=0.0, max_value=14.0, value=7.2, step=0.1)
    with c2:
        cl_libero = st.number_input("Cloro Libero (ppm)", min_value=0.0, value=1.0)
        cl_totale = st.number_input("Cloro Totale (ppm)", min_value=0.0, value=1.0)
    
    cl_combinato = max(0.0, cl_totale - cl_libero)
    st.info(f"💡 Cloro Combinato (CC): **{cl_combinato:.2f} ppm**")
    
    if st.button("🚀 GENERA REPORT DOSAGGI", use_container_width=True):
        st.markdown('<p class="titolo-sezione">Dosaggi Consigliati</p>', unsafe_allow_html=True)
        if ph_ril > 7.4:
            st.markdown(f'<div class="result-box">👉 pH meno G: <span class="misura-grande">{(v_piscina*10*((ph_ril-7.2)/0.1))/1000:.2f} kg</span></div>', unsafe_allow_html=True)
        if cl_combinato >= 0.4:
            ppm_shock = (cl_combinato * 10) - cl_libero
            st.markdown(f'<div class="result-box">🔥 Shock Chemacal 70: <span class="misura-grande">{(v_piscina*1.5*ppm_shock)/1000:.2f} kg</span></div>', unsafe_allow_html=True)

# --- TAB 2: SOLUZIONE ---
with tab2:
    st.header("Preparazione Soluzione Vasca")
    v_v = st.number_input("Volume Vasca Soluzione (L)", value=100.0)
    l_i = st.number_input("Litri prodotto versati (L)", value=10.0)
    p_p = st.number_input("% Prodotto Commerciale", value=15.0)
    st.success(f"### ✅ Valore in programmazione: {(l_i/v_v)*p_p if v_v > 0 else 0:.2f} %")

# --- TAB 3: GESTIONE (Replica Light Grafica) ---
with tab3:
    st.header("🚰 Verifica Macchina Installata")
    st.markdown('<p class="titolo-sezione">Parametri Impianto</p>', unsafe_allow_html=True)
    g1, g2 = st.columns(2)
    with g1:
        vr_g = st.number_input("Volume Resina (Litri)", value=25, key="vr_g")
        di_g = st.number_input("Durezza Acqua Grezza (°f)", value=35, key="di_g")
    with g2:
        do_g = st.number_input("Durezza Miscelata Uscita (°f)", value=15, key="do_g")
        co_g = st.number_input("Consumo Acqua Giornaliero (m³)", value=0.6, key="co_g")

    delta_g = max(0.1, di_g - do_g)
    cap_c_g = vr_g * 5 #
    m3_c_g = cap_c_g / delta_g #
    s_rig_g = (vr_g * 140) / 1000 #
    
    st.markdown('<p class="titolo-sezione">Risultati Tecnici</p>', unsafe_allow_html=True)
    res1, res2 = st.columns(2)
    res1.metric("Produzione Acqua Dolce", f"{m3_c_g:.2f} m³", f"Ogni {m3_c_g/co_g:.1f} gg") #
    res2.metric("Consumo Sale", f"{s_rig_g:.2f} kg", "A rigenerazione") #

    st.markdown("---")
    d1, d2, d3 = st.columns(3)
    d1.write(f"<span class='stat-label'>Cap. Ciclica</span><br><span class='stat-value'>{cap_c_g} m³f</span>", unsafe_allow_html=True) #
    d2.write(f"<span class='stat-label'>Volume Salamoia</span><br><span class='stat-value'>{(s_rig_g*3):.1f} L</span>", unsafe_allow_html=True) #
    d3.write(f"<span class='stat-label'>Acqua Scarico</span><br><span class='stat-value'>{vr_g*7} L</span>", unsafe_allow_html=True) #

# --- TAB 4: PROGETTO UNI 9182 & RISPARMIO ---
with tab4:
    st.header("📐 Progettazione e Analisi Portate")
    
    p1, p2 = st.columns(2)
    with p1:
        tipo = st.selectbox("Tipo Utenza", ["Villetta", "Condominio"], key="p_tipo")
        p_in = st.number_input("Durezza Ingresso (°f)", value=35, key="p_in")
        p_out = st.number_input("Durezza Uscita desiderata (°f)", value=15, key="p_out")
    with p2:
        if tipo == "Villetta":
            pers = st.number_input("Numero Persone", value=4, key="p_p")
            fabb_gg = pers * 0.20 #
            q_picco = 1.20 #
        else:
            app = st.number_input("Numero Appartamenti", value=10, key="p_a")
            fabb_gg = (app * 3) * 0.20 #
            q_picco = round(0.20 * math.sqrt(app * 3) + 0.8, 2) #

    p_delta = max(0.1, p_in - p_out)
    res_nec = (fabb_gg * 3 * p_delta) / 5 # Dimensionamento su 3gg autonomia
    taglie = [8, 12, 15, 20, 25, 30, 40, 50, 75, 100, 125, 150, 200, 250, 300]
    scelta = min([t for t in taglie if t >= res_nec] or [max(taglie)]) #
    
    # Calcoli Comparativi Annuali
    m3_autonomia = (scelta * 5) / p_delta #
    n_rig_anno = 365 / (m3_autonomia / fabb_gg) #
    
    # Valvola Standard
    sale_std = (scelta * 0.14) * n_rig_anno #
    acqua_std = (scelta * 7 * n_rig_anno) / 1000 #
    
    # Clack Impression (Risparmio medio 20%)
    sale_clack = sale_std * 0.80 #
    acqua_clack = acqua_std * 0.80 #

    st.markdown(f'<div class="result-box"><span class="nome-prodotto">Taglia Addolcitore Suggerita:</span><br><span class="valore-evidenziato" style="font-size:45px !important;">{scelta} Litri Resina</span></div>', unsafe_allow_html=True) #

    st.markdown('<p class="titolo-sezione">Dati di Progetto e Portate (UNI 9182)</p>', unsafe_allow_html=True)
    c_pic1, c_pic2 = st.columns(2)
    c_pic1.metric("Consumo Giornaliero", f"{fabb_gg:.2f} m³/gg") #
    c_pic2.metric("Portata di Picco", f"{q_picco} m³/h") #

    st.markdown('<p class="titolo-sezione">📊 Confronto Annuale: Standard vs Clack</p>', unsafe_allow_html=True)
    st.table({
        "Dati Annuali": ["Sale Totale", "Sacchi (25kg)", "Acqua Scarico", "N. Rigenerazioni"],
        "Valvola Standard": [f"{sale_std:.0f} kg", f"{math.ceil(sale_std/25)}", f"{acqua_std:.2f} m³", f"{n_rig_anno:.0f}"], #
        "Clack Impression": [f"{sale_clack:.0f} kg", f"{math.ceil(sale_clack/25)}", f"{acqua_clack:.2f} m³", "Ottimizzata"] #
    })

    st.success(f"📉 **Risparmio stimato con Clack:** {sale_std - sale_clack:.0f} kg di sale e { (acqua_std - acqua_clack)*1000 :.0f} L d'acqua ogni anno.") #
    
    if q_picco > 3.0:
        st.warning(f"⚠️ Attenzione: La portata di picco ({q_picco} m³/h) richiede valvole da 1\" 1/4 o superiore.") #
