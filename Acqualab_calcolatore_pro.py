...
# --- 3. TAB GESTIONE (CON CONSUMI SINGOLI E INTERVALLO) ---
with tab3:
    st.header("🚰 Verifica Macchina Installata")
    g1, g2 = st.columns(2)
    with g1:
        vr = st.number_input("Volume Resina (L)", value=25)
...
# --- 4. TAB PROGETTO (PORTATA E CONFRONTO ANNUALE) ---
with tab4:
    st.header("📐 Progettazione UNI 9182")
    pr1, pr2 = st.columns(2)
    with pr1:
        tipo = st.selectbox("Tipo Utenza", ["Villetta", "Condominio"])
        p_in = st.number_input("Durezza In (°f)", value=35, key="p_in")
        p_out = st.number_input("Durezza Out (°f)", value=15, key="p_out")
    with pr2:
        if tipo == "Villetta":
    ...
    st.subheader("Dati di Progetto e Portate")
    cp_res1, cp_res2 = st.columns(2)
    cp_res1.metric("Consumo Giornaliero", f"{f_gg:.2f} m³/gg")
    cp_res2.metric("Portata di Picco", f"{q_p} m³/h")

    st.subheader("📊 Confronto Annuale: Standard vs Clack")
    n_rig = 365 / (((taglia * 5) / p_delta) / f_gg)
    s_std = (taglia * 0.14) * n_rig
    st.table({
        "Dati Annuali": ["Sale Totale", "Sacchi (25kg)", "Acqua Scarico", "N. Rigenerazioni"],
        "Valvola Standard": [f"{s_std:.0f} kg", f"{math.ceil(s_std/25)}", f"{(taglia*7*n_rig)/1000:.2f} m³", f"{n_rig:.0f}"],
