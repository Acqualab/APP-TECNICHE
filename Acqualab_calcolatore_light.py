...
with tab2:
    ...
    if st.button("🚀 CALCOLA INTERVENTI", type="primary", use_container_width=True):
        ...
        if ph_ril > 7.2:
            diff = (ph_ril - 7.2) / 0.1
            st.warning("**pH ALTO.** Quantità da inserire:")
            st.markdown(f"👉 Carisan pH meno G: ### **{(v_piscina*10*diff)/1000:.2f} kg**")
            st.markdown(f"👉 Carisan pH meno L 15%: ### **{(v_piscina*27*diff)/1000:.2f} L**")
            st.markdown(f"👉 Carisan pH meno L 40%: ### **{(v_piscina*9*diff)/1000:.2f} L**")
        elif ph_ril < 7.2 and ph_ril > 0:
            diff = (7.2 - ph_ril) / 0.1
            st.info(f"**pH BASSO.** Inserire:")
            st.markdown(f"👉 pH Plus: ### **{(v_piscina*10*diff)/1000:.2f} kg**")
        else:
        ...
        if cl_ril < 1.5:
            d_cl = 1.5 - cl_ril
            st.error("**Cloro BASSO.** Quantità da inserire:")
            st.markdown(f"👉 Chemacal 70: ### **{(v_piscina*1.5*d_cl)/1000:.2f} kg**")
            st.markdown(f"👉 Power Clor 56: ### **{(v_piscina*1.8*d_cl)/1000:.2f} kg**")
            st.markdown(f"👉 Chemaclor L: ### **{(v_piscina*7*d_cl)/1000:.2f} L**")
        else:
        ...
        if cya_reale < 30:
            st.markdown(f"👉 Dose Acido Cianurico: ### **{(v_piscina*(30-cya_reale))/1000:.2f} kg**")
        
        st.markdown(f"👉 Alghicida (Mantenimento): ### **{(v_piscina*5)/1000:.2f} L/sett**")
