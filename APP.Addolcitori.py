...
with tab1:
    ...
    dur_abb = max(0.1, dur_in - dur_out)
    cap_c = v_resina * 5 
    m3_c = cap_c / dur_abb
    gg_a = m3_c / cons_m3
...
with tab2:
    ...
    with cp1:
        ...
        d_ing = st.number_input("Durezza Acquedotto (°f)", value=35, key="p_di")
    with cp2:
        d_mix = st.number_input("Durezza Mix Desiderata (°f)", value=15, key="p_dm")
        if tipo_ut == "Villetta":
    ...
    st.success(f"🎯 **Taglia Suggerita: {t_p} Litri**")
    st.info(f"Intervallo Rigenerazione: Ogni {gg_p:.1f} giorni")

    n_rig = 365 / gg_p if gg_p > 0 else 0
    s_std = (t_p * 0.14) * n_rig
    
    st.table({
        "Parametro": ["Sale (kg/anno)", "Sacchi (25kg)", "Scarico (m³/anno)", "Portata Picco"],
        "Standard": [f"{s_std:.0f}", f"{math.ceil(s_std/25)}", f"{(t_p*7*n_rig)/1000:.1f}", f"{p_picco} m³/h"],
        "Tecno Clack": [f"{s_std*0.75:.0f}", f"{math.ceil(s_std*0.75/25)}", f"{(t_p*5*n_rig)/1000:.1f}", f"{p_picco} m³/h"]
    })
